...
@section('box')
<div>
    <p class="font-medium text-3xl pb-4">Edit Address</p>
...
<form x-data="editAddressForm" @submit.prevent="submit" class="mt-4 flex-col">
    <div class="pt-2">
...
document.addEventListener('alpine:init', () => {
  Alpine.data('editAddressForm', () => ({
    address: '{{ $deliveryAddress->address }}',
    phone_number: '{{ $deliveryAddress->phone_number }}',
    is_default: {{ $deliveryAddress->is_default ? 'true' : 'false' }},
    submit() {
      axios.post(route('profile.delivery-addresses.update', {{ $deliveryAddress->id }}), {
        _method: 'PUT',
