...
@else
    <div class="relative mt-1" x-data="{ open: false, value: null, label: '{!! $defaultLabel !!}' }">
    ...
    <ul x-transition x-cloak x-show="open" class="absolute z-10 w-full py-1 mt-1 overflow-auto bg-white rounded-md shadow-lg max-h-56 ring-1 ring-black ring-opacity-5 focus:outline-none">
        ...
        @foreach ($options as $text => $value)
            <li x-data="{ optionValue: '{{ $value }}' }" @click="label = '{{ $text }}'" class="relative py-2 pl-3 text-gray-900 cursor-pointer select-none hover:bg-gray-50 pr-9" id="listbox-option-0" role="option">
                <div class="flex items-center">
